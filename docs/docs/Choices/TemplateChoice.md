...
QuickAdd supports markdown (`.md`), canvas (`.canvas`), and base (`.base`) templates. It also reads configured source-only extensions, with `.eta` enabled by default for Templater/Eta workflows. You can add or remove source-only extensions in [Settings → Templates & Properties](/Settings.md#templates--properties).
