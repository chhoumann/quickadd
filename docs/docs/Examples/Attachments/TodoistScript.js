...
const getTodoistPluginApi = (app) =>
	app.plugins.plugins["todoist-sync-plugin"].api;
...
async function GetAllTasksFromProject(params) {
	...
	if (targetProject.tasks.length === 0) {
		...
		new Notice(
			`Added ${targetProject.tasks.length} tasks from '${targetProject.name}'.`
		);
	}
...
async function GetAllTasksFromSection(params) {
	const [projects, sections, allTasks] = await Promise.all([
		getProjects(params.app),
		getSections(params.app),
		getAllTasks(params),
	]);

	const targetSection = await params.quickAddApi.suggester((section) => {
		const sectionProject = projects.find(
			(project) => project.id === section.project_id
		);
	...
	if (targetSection.tasks.length === 0) {
		...
		new Notice(
			`Added ${targetSection.tasks.length} tasks from '${targetSection.name}'.`
		);
	}
...
async function selectTasks(params, tasks) {
	const selectedTaskNames = await params.quickAddApi.checkboxPrompt(
		tasks.map((task) => task.content)
	);
	return tasks.filter((task) =>
		selectedTaskNames.some((t) => t.contains(task.content))
	);
}
...
function formatTasksToTasksPluginTask(tasks) {
	return `${tasks
		.map((task) =>
			task.rawDatetime
				? `- [ ] ${task.content} 📅 ${task.rawDatetime.format("YYYY-MM-DD")}`
				: `- [ ] ${task.content}`
		)
		.join("\n")}\n`;
}

async function _getTasksGroupedByProject(app) {
	const api = getTodoistPluginApi(app);
