...
module.exports = async function start(params) {
	...
	const clipBoardContents = await QuickAdd.quickAddApi.utility.getClipboard();
	const title = await QuickAdd.quickAddApi.inputPrompt(
		"Enter Book title: ",
		clipBoardContents,
		clipBoardContents // clipBoardContents is added once as the prompt text and once as the default value
	);
	if (!title) {
	...
	QuickAdd.variables = {
		...
		fileName: replaceIllegalFileNameCharactersInString(
			bookDesc.items[0].volumeInfo.title
		),
