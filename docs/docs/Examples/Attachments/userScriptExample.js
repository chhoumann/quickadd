	...
	settings: {
		...
		options: {
			...
			"File Name Pattern": {
				...
				description:
					"Template for generating file names (supports QuickAdd format syntax)",
			},
...
async function start(params, settings) {
	...
	const _apiKey = settings["API Key"];
	...
	const maxItems = parseInt(settings["Max Items"], 10);
	...
	if (!projectName) {
		new obsidian.Notice(
			"Please configure a project name in the script settings"
		);
	...
	try {
		...
		await quickAddApi.infoDialog("Project Created", [
			`Name: ${projectName}`,
			`Type: ${templateType}`,
			`Priority: ${priority}`,
			`Category: ${category}`,
