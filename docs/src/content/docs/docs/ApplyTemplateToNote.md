...
Only markdown templates can be applied: canvas (`.canvas`) and base (`.base`) templates contain data for their own file types and are excluded from the picker (use a regular [Template choice](/docs/Choices/TemplateChoice/) to create those). Likewise, the target must be a markdown note.
...
The [QuickAdd API](/docs/QuickAddAPI/) exposes the same functionality without any prompts:
