...
Option-form default value, required when combining with other options like `|label:`. Example: `{{VALUE:title|label:Snake case|default:My_Title}}`.
...
Generates a random alphanumeric string of the specified length (1-100). Useful for creating unique identifiers, block references, or temporary codes. Example: `{{RANDOM:6}}` generates something like `3YusT5`.
