...
class TestTemplateEngine {
	...
	async testOverwriteFileWithTemplate(
		file: TFile,
		templatePath: string
	): Promise<TFile | null> {
		try {
...
Updated content: {{content}}`;
			...
			await templateEngine.testOverwriteFileWithTemplate(
				existingFile,
				"templates/update.md"
			);
...
Test content`;
			...
			expect(capturedFrontmatter.testDate.toISOString()).toBe(
...
Content`;
			...
			mockChoiceExecutor.variables.set("mixedArray", [
				"string",
				42,
				true,
				null,
			...
			expect(capturedFrontmatter.mixedArray).toEqual([
				"string",
				42,
				true,
				null,
...
Content`;
			...
			mockChoiceExecutor.variables.set("simpleObject", {
				key: "value",
				count: 5,
			});
			...
			await templateEngine.testCreateFileWithTemplate(
				"notes/objects.md",
				"templates/objects.md"
			);

			expect(capturedFrontmatter.emptyObject).toEqual({});
			expect(capturedFrontmatter.simpleObject).toEqual({
				key: "value",
				count: 5,
			});
