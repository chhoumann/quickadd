...
describe("Template Property Types Feature Flag & Edge Cases", () => {
	...
	beforeEach(() => {
		...
		mockFileManager = {
			processFrontMatter: vi.fn().mockImplementation((_file, callback) => {
				const frontmatter = {};
	...
	describe("Feature Flag Behavior", () => {
		...
		it("should not post-process when flag is disabled", async () => {
			...
			const templateVars = new Map<string, unknown>([
				["title", "Test"],
				["count", 42],
			]);

			const shouldProcess = await engine.testShouldProcessFrontMatter(
				mockFile,
				templateVars
			);
	...
	describe("Error Handling", () => {
		it("should handle processFrontMatter YAML parse errors gracefully", async () => {
			...
			await expect(
				engine.testPostProcessFrontMatter(mockFile, templateVars)
	...
	describe("Edge Cases", () => {
		...
		it("should only process markdown files for front matter", async () => {
			...
			const shouldProcessCanvas = await engine.testShouldProcessFrontMatter(
				canvasFile,
				templateVars
			);
			...
			const shouldProcessMd = await engine.testShouldProcessFrontMatter(
				mdFile,
				templateVars
			);
...
class TestFormatter {
	...
	formatContent(template: string): string {
		...
		while (match !== null) {
			...
			if (
				...
				this.isInYamlKeyValuePosition(output, match.index)
			) {
	...
	private isInYamlKeyValuePosition(
		output: string,
		matchIndex: number
	): boolean {
...
class TestTemplateEngine {
	...
	async testPostProcessFrontMatter(
		file: TFile,
	...
	async testShouldProcessFrontMatter(
		file: TFile,
		templateVars: Map<string, unknown>
	): Promise<boolean> {
		return (
