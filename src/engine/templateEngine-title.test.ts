...
class TestTemplateEngine extends TemplateEngine {
	...
	public async testCreateFileWithTemplate(
		filePath: string,
		templatePath: string
	) {
...
describe("TemplateEngine - Title Handling", () => {
	...
	beforeEach(() => {
		...
		mockApp = {
			vault: {
				...
				create: vi
					.fn()
					.mockResolvedValue({ path: "test.md", basename: "test" }),
			},
			workspace: {
				getActiveFile: vi.fn(),
			},
	...
	describe("createFileWithTemplate", () => {
		...
		it("should extract title from path with folders", async () => {
			await engine.testCreateFileWithTemplate(
	...
	describe("formatFileName - title exclusion", () => {
		it("should NOT replace {{title}} in filename formatting", async () => {
			...
			const result = await mockFormatter.formatFileName(
				"{{title}}-note.md",
				""
			);
