...
class SystemTestEngine {
	...
	async overwriteFileWithTemplate(
		file: TFile,
		templatePath: string
	): Promise<TFile | null> {
		try {
	...
	private async postProcessFrontMatter(
		...
		const processedContent = this.processFrontMatterContent(
			currentContent,
			templateVars
		);
		...
		this.app.fileManager.processFrontMatter.mockImplementation(
			(_targetFile: TFile, fn: (frontmatter: any) => void) => {
	...
	private processFrontMatterContent(
		content: string,
		templateVars: Map<string, unknown>
	): string {
		const lines = content.split(/\r?\n/);
...
`;
			...
			it("should handle complex academic paper with all property types", async () => {
				...
				testEngine.setVariable(
					"abstract",
					"This study explores novel applications..."
				);
				testEngine.setVariable(
					"methodology",
					"We employed state-of-the-art neural networks..."
				);
				...
				const finalContent = testEngine.getFileContent(
					"papers/climate-ai-paper.md"
				);
				...
				expect(finalContent).toContain(
					'tags: ["climate", "deep-learning", "AI", "research"]'
				);
				expect(finalContent).toContain(
					'metadata: {"journal":"Nature Climate Change","volume":13,"issue":4,"pages":"123-135","doi":"10.1038/s41558-023-01234-5"}'
				);
...
`;
			...
			it("should handle nested project data structures", async () => {
				...
				expect(finalContent).toContain(
					'team_members: ["Alice (Lead Designer)", "Bob (Frontend Dev)", "Carol (Backend Dev)", "Dave (QA Engineer)"]'
				);
...
`;
			...
			it("should handle deeply nested data structures", async () => {
				...
				expect(finalContent).toContain(
					'"name":"analytics_db","type":"mongodb"'
				);
			});
		});
...
`;
			...
			it("should handle all supported property types correctly", async () => {
				...
				expect(finalContent).toContain(
					'author: {"name":"John Doe","email":"john@example.com","role":"Developer"}'
				);
				expect(finalContent).toContain(
...
`;
				...
				expect(content).toContain(
					"captured: This is important information that was captured."
				);
...
`;
		...
		it("should handle network/file system errors with proper fallback", async () => {
			...
			mockApp.vault.cachedRead.mockRejectedValueOnce(
...
`;
			...
			testEngine.setVariable("quickadd_array", [
				"quickadd",
				"templater",
				"test",
			]);
			...
			expect(content).toContain(
				'quickadd_array: ["quickadd", "templater", "test"]'
			);
...
`;
			...
			expect(content).toContain(
				"both_reference: Order Test - <% tp.file.title %>"
			);
