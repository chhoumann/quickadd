...
async function getFieldValues(
	...
	try {
		...
		log.logError(
			new Error(`Failed to query Dataview for field ${fieldName}: ${error}`)
		);
	}
 ...
 * @returns Set of unique field values
 */
async function getFieldValuesWithFilter(
	app: App,
	fieldName: string,
	folder?: string,
	tags?: string[],
	...
	try {
		...
		if (excludeTags && excludeTags.length > 0) {
			for (const excludeTag of excludeTags) {
				const tagName = excludeTag.startsWith("#")
					? excludeTag
					: `#${excludeTag}`;
				conditions.push(`!contains(file.tags, "${tagName}")`);
			}
		...
		log.logError(
			new Error(
				`Failed to query Dataview for field ${fieldName} with filters: ${error}`
			)
		);
	}
 ...
 */
function isAvailable(app: App): boolean {
	return getAPI(app) !== null;
}

export const DataviewIntegration = {
	getFieldValues,
	getFieldValuesWithFilter,
	isAvailable,
};
