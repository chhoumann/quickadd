...
export class FileSuggester extends TextInputSuggest<SearchResult> {
		...
		for (const file of candidateFiles) {
			...
			const filteredHeadings = headings
				.filter(
					(h) =>
						headingQuery === "" ||
						h.toLowerCase().includes(headingQuery.toLowerCase())
				)
				.slice(0, 20);
			...
			for (const heading of filteredHeadings) {
				results.push({
					...
					displayText: noFileSpecified
						? `#${heading}`
						: `${file.basename}#${heading}`,
				});
			}
		}
	...
	private getAttachmentSuggestions(query: string): SearchResult[] {
		...
		return attachmentFiles.slice(0, 20).map((file) => ({
			file: {
				path: file.path,
	...
	renderSuggestion(item: SearchResult, el: HTMLElement): void {
		...
		switch (matchType) {
			...
			case "heading": {
				...
				mainText =
					headingQuery &&
					heading.toLowerCase().includes(headingQuery.toLowerCase())
			...
			case "unresolved":
				...
				pill =
					'<span class="qa-suggestion-pill qa-unresolved-pill">unresolved</span>';
				break;
		}
	...
	async selectSuggestion(item: SearchResult): Promise<void> {
		...
		const isEmbedMode =
			this.inputEl.value.slice(
				cursorPosition - this.lastInput.length - 3,
				cursorPosition - this.lastInput.length
			) === "![[";
