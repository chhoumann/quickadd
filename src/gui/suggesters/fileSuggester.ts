...
function buildFileMap(files: AliasArrTFile[], unresolvedLinkNames: string[]): SuggestionMap {
...
function getAliasesForFile(file: TFile, app: App): AliasArrTFile {
    ...
    const fileMetaAlias: (string|string[]) = fileMetadata?.frontmatter?.alias ?? fileMetadata?.frontmatter?.aliases ?? "";
    let aliases: string[] = [];
...
export class SilentFileSuggester extends TextInputSuggest<string> {
    ...
    constructor(public app: App, public inputEl: HTMLInputElement | HTMLTextAreaElement) {
        ...
        const filesAndAliases: AliasArrTFile[] = app.vault.getMarkdownFiles().map(file => getAliasesForFile(file, app));
