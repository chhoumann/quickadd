...
import { collectFieldValuesProcessed, generateFieldCacheKey } from "src/utils/FieldValueCollector";
...
export class FieldValueInputSuggest extends TextInputSuggest<string> {
  ...
  async getSuggestions(inputStr: string): Promise<string[]> {
    if (!this.cachedValues) {
      this.cachedValues = await collectFieldValuesProcessed(this.app, this.fieldName, this.filters);
    }
