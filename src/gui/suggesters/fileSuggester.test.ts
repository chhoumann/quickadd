...
describe("FileSuggester - Issue #838 and #839", () => {
	...
	beforeEach(async () => {
		...
		files = [
			...
			createMockFile(
				"Archive/General Business Terms.md",
			...
			createMockFile(
				"Projects/Project Planning Guide.md",
	...
	describe("Issue #838 - Short exact identifiers appear first", () => {
		...
		it("should rank exact match first even when other files have similar prefixes", () => {
			...
			const gbtFiles = topResults.filter((f) =>
				f.file.basename.toLowerCase().startsWith("gbt")
			);
	...
	describe("Issue #839 - Exact title matches always rank first", () => {
		...
		it("should rank exact match first even with longer similar files", () => {
			...
			const planningIndex = results.findIndex(
				(f) => f.file.basename === "Planning"
			);
	...
	describe("Tier-based ranking", () => {
		...
		it("should prioritize prefix matches over fuzzy matches", () => {
			...
			const projectPlanningIndex = results.findIndex(
				(f) => f.file.basename === "Project Planning Guide"
			);
	...
	describe("Basename-exact vs Alias-exact ranking", () => {
		it("should rank basename-exact match before alias-exact match", async () => {
			...
			vi.mocked(mockApp.metadataCache.getFileCache).mockImplementation(
				(file: TFile) => {
			...
			// Verify ordering
			const basenameIndex = results.findIndex(
				(r) => r.file.basename === "MyFile"
			);
...
describe("FileSuggester - Substring search with long aliases", () => {
	...
	beforeEach(async () => {
		...
		files = [
			...
			createMockFile(
				"Projects/Project Beta Documentation.md",
				"Project Beta Documentation"
			),
			createMockFile("Archive/Old Project Notes.md", "Old Project Notes"),
			createMockFile(
				"Planning/2025 Project Roadmap.md",
				"2025 Project Roadmap"
			),
			createMockFile(
				"References/Project Management Guide.md",
			...
			(file: TFile) => {
				if (file.basename === "Project Beta Documentation") {
					return {
						frontmatter: {
							aliases: [
								"Complete documentation for Project Beta including all technical specifications and implementation details",
							],
						},
					};
				}
				if (file.basename === "2024-03-15") {
					return {
						frontmatter: {
							aliases: [
								"Daily review of all ongoing projects and their current status updates",
							],
						},
					};
				}
				if (file.basename === "Old Project Notes") {
					return {
						frontmatter: {
							aliases: [
								"Archived notes from previous projects that might be useful for reference",
							],
						},
					};
				}
	...
	it("should prioritize basename substring matches over long alias matches", () => {
		...
		const projectAlpha = results.findIndex(
			(r) => r.file.basename === "Project Alpha"
		);
		const projectRoadmap = results.findIndex(
			(r) => r.file.basename === "2025 Project Roadmap"
		);

		// Files that only match via long aliases should appear later
		const dailyNote = results.findIndex(
			(r) => r.file.basename === "2024-03-15"
		);
		...
		const basenameMatches = results.filter((r) =>
			r.file.basename.toLowerCase().includes("project")
		);
		const aliasOnlyMatches = results.filter(
			(r) =>
				r.matchType === "alias" &&
				!r.file.basename.toLowerCase().includes("project")
		);
		...
		if (basenameMatches.length > 0 && aliasOnlyMatches.length > 0) {
			const worstBasenameScore = Math.max(
				...basenameMatches.map((m) => m.score)
			);
