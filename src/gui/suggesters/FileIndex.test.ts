...
describe("FileIndex", () => {
	...
	describe("scoring system", () => {
		it("should boost same-folder files", () => {
			...
			const score = (fileIndex as any).calculateScore(
				mockFile,
				"test",
				context,
				0.5
		...
		it("should rank basename matches better than alias matches", () => {
			...
			const aliasScore = (fileIndex as any).calculateScore(
				mockFile,
				"my-alias",
				{},
				0.5,
			...
			const basenameScore = (fileIndex as any).calculateScore(
				mockFile,
				"test",
				{},
				0.5,
	...
	describe("basic functionality", () => {
		...
		it("should use incremental updates for single file changes", async () => {
			...
			const updateFuseIndexSpy = vi.spyOn(
				freshIndexWithPrivates,
				"updateFuseIndex"
			);
			const processPendingUpdatesSpy = vi.spyOn(
				freshIndexWithPrivates,
				"processPendingFuseUpdates"
			);
		...
		it("should batch multiple rapid updates", async () => {
			...
			expect(indexWithPrivates.pendingFuseUpdates.get("file2.md")).toBe(
				"update"
			);
		});
		...
		it("should handle remove+add sequence as update", () => {
			...
			expect(indexWithPrivates.pendingFuseUpdates.get("file.md")).toBe(
				"update"
			);
		});
	});
			...
			const files = [
				{
					path: "file1.md",
					basename: "file1",
					headings: ["Introduction", "My Heading", "Conclusion"],
				},
				{
					path: "file2.md",
					basename: "file2",
					headings: ["Overview", "My Special Heading"],
				},
			];
	...
	describe("search functionality", () => {
		it.skip("should return exact matches first", async () => {
			...
			const exactMatch = results.find(
				(r) => r.file.basename === "test" && r.matchType === "exact"
			);
		...
		it.skip("should handle relative path suggestions", async () => {
			...
			const folder1Result = results.find(
				(r) => r.file.path === "folder1/note.md"
			);
			const folder2Result = results.find(
				(r) => r.file.path === "folder2/note.md"
			);
