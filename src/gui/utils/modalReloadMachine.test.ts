...
describe("ModalReloadController", () => {
	...
	it("drains a queued reload request raised during render", () => {
		const harness = createModalHarness();
		let controller: ModalReloadController | null = null;
		try {
			const transitions: string[] = [];
			...
			const reloadController = new ModalReloadController({
				...
				render: () => {
					renderCount += 1;
					renderSettingWithInput(harness.contentEl, {
						settingName: "Queued reload test",
					});
					if (!queued) {
						queued = true;
						controller?.requestReload("queued");
					}
				},
				onTransition: (_from, to, reason) => {
					if (to === "capturing") {
			...
			reloadController.requestReload("initial");

			expect(renderCount).toBe(2);
			expect(transitions).toEqual(["initial", "queued"]);
		} finally {
			controller?.destroy();
			harness.destroy();
		}
	...
	it("coalesces multiple queued reload requests to the latest reason", () => {
		const harness = createModalHarness();
		let controller: ModalReloadController | null = null;
		try {
			const transitions: string[] = [];
			...
			const reloadController = new ModalReloadController({
				...
				render: () => {
					renderCount += 1;
					renderSettingWithInput(harness.contentEl, {
						settingName: "Coalesce test",
					});
					if (!queued) {
						queued = true;
						controller?.requestReload("queued-1");
						controller?.requestReload("queued-2");
					}
				},
				onTransition: (_from, to, reason) => {
					if (to === "capturing") {
			...
			expect(renderCount).toBe(2);
			expect(transitions).toEqual(["initial", "queued-2"]);
		} finally {
			controller?.destroy();
			harness.destroy();
		}
