...
export abstract class TextPromptModal<
	Field extends TextComponent | TextAreaComponent,
> extends Modal {
	...
	protected inputComponent: Field;
	...
	private display() {
		this.containerEl.addClass(
			"quickAddModal",
			this.getPromptKind() === "single" ? "qaInputPrompt" : "qaWideInputPrompt",
		);
	...
	protected abstract getPromptKind(): "single" | "multi";

	protected abstract createInputField(
	...
	protected configureInput(
		...
		field.inputEl.addEventListener("keydown", this.submitEnterCallback);
	...
	protected abstract shouldSubmit(evt: KeyboardEvent): boolean;
	 ...
	 */
	protected skip() {
		if (this.getPromptKind() === "multi" && this.didSubmit) return;
		this.input = "";
		this.didSubmit = true;
