...
describe("VDATE Default Value Support", () => {
	...
	beforeEach(() => {
		...
		global.window = {
			//@ts-expect-error
			moment: vi.fn((_isoString: string) => ({
				isValid: () => true,
	...
	describe("Regex Pattern", () => {
		...
		it("should match VDATE with complex default values", () => {
			const testCases = [
				{
					input: "{{VDATE:date,YYYY-MM-DD|next monday}}",
					defaultValue: "next monday",
				},
				{ input: "{{VDATE:date,YYYY-MM-DD|+7 days}}", defaultValue: "+7 days" },
				{
					input: "{{VDATE:date,YYYY-MM-DD|2025-12-25}}",
					defaultValue: "2025-12-25",
				},
				{
					input: "{{VDATE:date,YYYY-MM-DD|tomorrow at 3pm}}",
	...
	describe("Default Value Processing", () => {
		...
		it("should use user input over default value", async () => {
			...
			expect(formatter.testDateParser.parseDate).toHaveBeenCalledWith(
				"tomorrow"
			);
			expect(result).toBe("Test YYYY-MM-DD-formatted");
		});
		...
		it("should handle multiple VDATE variables with different defaults", async () => {
			const input =
				"Start: {{VDATE:start,YYYY-MM-DD|today}} End: {{VDATE:end,YYYY-MM-DD|next week}}";
			formatter.setMockPromptValue("");

			const result = await formatter.testReplaceDateVariableInString(input);

			expect(result).toBe(
				"Start: YYYY-MM-DD-formatted End: YYYY-MM-DD-formatted"
			);
			expect(formatter.testDateParser.parseDate).toHaveBeenCalledWith("today");
			expect(formatter.testDateParser.parseDate).toHaveBeenCalledWith(
				"next week"
	...
	describe("Variable Reuse", () => {
		it("should reuse the same variable with different formats and respect the default only on first use", async () => {
			const input =
				"Date1: {{VDATE:date,YYYY-MM-DD|today}} Date2: {{VDATE:date,MM/DD/YYYY|tomorrow}}";
			formatter.setMockPromptValue("");
			...
			expect(formatter.testDateParser.parseDate).toHaveBeenCalledWith("today");
			expect(result).toBe(
				"Date1: YYYY-MM-DD-formatted Date2: MM/DD/YYYY-formatted"
			);
		});
	});
