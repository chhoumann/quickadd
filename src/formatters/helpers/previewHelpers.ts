...
export function getMacroPreview(macroName: string): string {
	const description =
		MACRO_DESCRIPTIONS[macroName.toLowerCase()] || `${macroName}_output`;
	return description;
}
 ...
 */
export function getCurrentFileLinkPreview(
 ...
 */
export namespace DateFormatPreviewGenerator {
	const MONTH_NAMES = [
		"January",
		"February",
		"March",
	 ...
	 */
	export function generate(format: string, date: Date = new Date()): string {
		...
		return (
			format
				...
				.replace(/MMM/g, MONTH_NAMES_SHORT[date.getMonth()])
				.replace(/MM/g, month)
				.replace(/M/g, (date.getMonth() + 1).toString())
				// Day patterns
				.replace(/dddd/g, DAY_NAMES[date.getDay()])
				.replace(/ddd/g, DAY_NAMES_SHORT[date.getDay()])
				.replace(/DD/g, day)
				...
				// Week patterns
				.replace(/ww/g, getWeekNumber(date).toString().padStart(2, "0"))
				.replace(/w/g, getWeekNumber(date).toString())
		);
	}
}
