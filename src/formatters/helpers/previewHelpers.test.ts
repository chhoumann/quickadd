...
describe("PreviewHelpers", () => {
	...
	describe("DateFormatPreviewGenerator", () => {
		...
		it("should format year patterns correctly", () => {
			expect(DateFormatPreviewGenerator.generate("YYYY", testDate)).toBe(
				"2024"
			);
		...
		it("should format month patterns correctly", () => {
			...
			expect(DateFormatPreviewGenerator.generate("MMMM", testDate)).toBe(
				"January"
			);
		});
		...
		it("should format complex patterns correctly", () => {
			const result = DateFormatPreviewGenerator.generate(
				"YYYY-MM-DD HH:mm",
				testDate
			);
