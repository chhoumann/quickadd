...
describe("RequirementCollector", () => {
	it("collects VALUE with default and options", async () => {
		...
		await rc.scanString(
			"{{VALUE:title|Untitled}} and {{VALUE:low,medium,high}}"
		);
	...
	it("collects VDATE with format and default", async () => {
		...
		const [due] = Array.from(rc.requirements.values()).filter(
			(r) => r.id === "due"
		);
	...
	it("records TEMPLATE references for recursive scanning", async () => {
		...
		expect(
			rc.templatesToScan.size === 0 || rc.templatesToScan.has("Templates/Note")
		).toBe(true);
	});
});
