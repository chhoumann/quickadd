...
let cachedDirectory: { data: ModelsDevDirectory; fetchedAt: number } | null =
	null;
const ONE_DAY_MS = 24 * 60 * 60 * 1000;

export async function fetchModelsDevDirectory(): Promise<ModelsDevDirectory> {
	if (cachedDirectory && Date.now() - cachedDirectory.fetchedAt < ONE_DAY_MS) {
...
export function mapEndpointToModelsDevKey(endpoint: string): string | null {
	...
	if (url.includes("llama.com") || url.includes("api.llama.com"))
		return "llama";
