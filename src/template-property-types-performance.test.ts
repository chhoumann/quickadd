...
class MockFormatter {
	...
	async formatFileContent(content: string): Promise<string> {
		...
		let match: RegExpExecArray | null = variableRegex.exec(content);
...
class MockTemplateEngine {
	...
	async processTemplate(
		template: string
	): Promise<{ file: TFile; templateVars: Map<string, unknown> }> {
	...
	async postProcessFrontMatter(
		file: TFile,
		templateVars: Map<string, unknown>
	): Promise<void> {
		// Simulate Obsidian's processFrontMatter call
		const processFrontMatter = this.mockApp.fileManager
			.processFrontMatter as MockedFunction<any>;
...
describe("Template Property Types Performance Tests", () => {
	...
	beforeEach(() => {
		...
		processFrontMatterSpy = vi.fn().mockImplementation((_file, callback) => {
	...
	describe("YAML Processing Performance", () => {
		...
		test("date conversion performance should be consistent", async () => {
			...
			for (let i = 0; i < 100; i++) {
				dateVars.set(
					`date${i}`,
					`@date:2024-01-${String((i % 31) + 1).padStart(2, "0")}T00:00:00Z`
				);
		...
		test("mixed feature flag toggling during processing should not affect performance", async () => {
			...
			const { duration } = await measurePerformance(async () => {
				for (let i = 0; i < 20; i++) {
					...
					if (
						mockPlugin.settings.enableTemplatePropertyTypes &&
						templateVars.size > 0
					) {
						await templateEngine.postProcessFrontMatter(file, templateVars);
					}
